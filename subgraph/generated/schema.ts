...
export class NftMeta extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save NftMeta entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type NftMeta must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("NftMeta", id.toString(), this);
    }
  }

  static load(id: string): NftMeta | null {
    return changetype<NftMeta | null>(store.get("NftMeta", id));
  }
